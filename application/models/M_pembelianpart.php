<?php
class M_pembelianpart extends CI_Model
{
    private $_table = "pembelian_part";
    ...
    public function buat_kode()   
    {
        ...
        $this->db->select('RIGHT(pembelian_part.kd_pemb,6) as kd_pemb', FALSE);
        $this->db->order_by('kd_pemb','DESC');
        $this->db->limit(1);
        $query = $this->db->get('pembelian_part');     
          if($query->num_rows() <> 0){
            $data = $query->row();
