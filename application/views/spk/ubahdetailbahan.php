...
<div class="container-fluid">
		...
		<h3><i class="fa fa-bookmark"></i>  Detail Bahan</h3>
	<div class="row">
		<div class="col-lg-12">
			<div class="card border-secondary" >
				...
				<div class="card-body">
				<a href="<?= base_url('spk/post/'); ?><?= $spk['no_est'] ?>" class="btn btn-success" style="margin-left:1225px;position:absolute;" onclick="return confirm('Apakah kamu yakin data akan di post?');">Post</a>
					<div class="row">
                         	...
                         	<div class="form-inline">
								...
								<div class="col-sm-3">
									...
									<a href="javascript:void(0);" NAME="My Window Name" title=" My title here "
										onClick='javascript:window.open("<?= base_url('spk/popbahandata'); ?>","Ratting",
								...
								<div class="form-inline">
									<div class="input-group sm-4">
										<input class="form-control" type="text" name="kd_jenis" value="<?= $dtl['kd_jenis'] ?>" id="kd_jenis" hidden style="width:225px;">
										<input class="form-control" type="text" name="kd_detail" value="<?= $dtl['kd_detail'] ?>" id="kd_detail" hidden style="width:225px;">
							...
							<div class="form-inline">
								<label class="" for="kode">Pekerjaan/Part</label>&nbsp &nbsp
									...
									<?php
										...
										?>
										<input class="form-control" type="text" name="jenis_pekerjaan" value="<?= $b['nama'] ?>" id="jenis_pekerjaan" style="width:230px;" autocomplete="off" readonly>
										<?php } ?>
										<?php endforeach ?>
									</div>
								<label class="col-sm" for="kode">Harga</label>
									<div class="col-xs-1">
										<input class="form-control" type="text" name="harga" value="<?= $dtl['harga'] ?>" id="harga" style="width:120px;" onkeyup="sum();" autocomplete="off">
									</div>
								<label class="col-sm" for="id">Jumlah</label>
									<div class="col-xs-1">
										<input class="form-control" type="text" name="jumlah" value="<?= $dtl['jumlah'] ?>" id="jumlah" style="width:60px;" onkeyup="sum();" autocomplete="off">
								...
								<label class="col-sm" for="id">Diskon</label>
									<div class="col-xs-1">
										<input class="form-control" type="text" name="diskon" value="<?= $dtl['diskon'] ?>" id="diskon" style="width:60px;" onkeyup="sum();" autocomplete="off">
									</div>
								<label class="col-sm" for="id">Total</label>
									<div class="col-xs-1">
										<input class="form-control" type="text" name="total" value="<?= $dtl['total'] ?>" id="total" style="width:150px;">
									</div>
			...
			<br>
				<div class="class table-responsive">
					<table class="table table-striped table-bordered" style="text-align:center;">
						...
						<tbody>
							<tr>
									...
									foreach ($detail as $dtl):
										if ($dtl['no_spk']== $spk['no_est']){ 
											foreach ($bahan as $pekerjaan):
												if ($pekerjaan['kd_bahan']== $dtl['kd_jenis']){
								?>
								...
								<td style="font-size:0.9rem"><?= $dtl['total']?></td>
								<td  style="width:50px;">
								<a href="<?= base_url('spk/ubahDetailBahan/'); ?><?= $spk['no_est']; ?>/<?= $dtl['kd_detail']; ?>" class="badge badge-secondary pl-2 pr-2">Edit</a>
					...
					<div class="row">
						...
						<div class="col-sm-2">
							<a href="/body-repair/spk/bahan" class="btn btn-danger float-right btn-block"><i class="fa fa-undo"></i> Kembali</a>
						</div>
					</div>
				<br>
			</div>
...
<script>
      function sum() {
		...
		var stok = parseFloat(jumlah2)-parseFloat(jumlah);
		var result = parseFloat(harga) * parseFloat(jumlah);
		var diskon = (parseFloat(result) * parseFloat(diskon))/ 100;
		var jumlah = parseFloat(result)-parseFloat(diskon);
