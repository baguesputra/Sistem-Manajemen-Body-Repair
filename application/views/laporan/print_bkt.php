...
<h5 align="center">BUKTI KAS TERIMA</h5>
<table>
    <tr>
        
            <td style="font-size:0.9rem">No Dokumen</td>
            <td style="font-size:0.9rem">: <?= $bkt['no_dok'] ?></td>
       
    </tr>
    <tr>
        
            <td style="font-size:0.9rem">Tanggal</td>
            <td style="font-size:0.9rem">: <?= date('d M Y', strtotime($bkt['tgl'])); ?></td>
       
    </tr>
    <tr>
        
        <td style="font-size:0.9rem">Customer</td>
        <td style="font-size:0.9rem">: <?= $bkt['customer'] ?></td>
...
<table style="position:absolute;margin-left:400px;margin-top:-70px;">
                <tr>
                    ...
                    <td style="font-size:0.9rem;">: <?= $bkt['gl_account'] ?></td>
                </tr>
                <tr>
                    <td style="font-size:0.9rem;">Deskripsi</td>
                    <?php foreach($account as $acc){
                        if($acc['no_account'] == $bkt['gl_account']){ ?>
                ...
                <table width="100%">
                    <tr>
                        ...
                        foreach ($detailbbkt as $detail) : 
                            if ($detail['kode_bbkt'] == $bkt['no_dok']){    
                        ?>
...
<table>
    <tr>
        ...
        <td style="font-size:0.9rem">: Rp.<?= number_format($bkt['penerimaan'], 0, ',', '.') ?></td>
   
    </tr>
    <tr>
        
        <td style="font-size:0.9rem">Terbilang</td>
        <td style="font-size:0.9rem">: <?= strtoupper(terbilang($bkt['penerimaan'])) ?> RUPIAH</td>
