...
<h5 align="center">BUKTI KAS KELUAR</h5>
<table>
    <tr>
        
            <td style="font-size:0.9rem">No Dokumen</td>
            <td style="font-size:0.9rem">: <?= $bkk['no_dok'] ?></td>
       
    </tr>
    <tr>
        
            <td style="font-size:0.9rem">Tanggal</td>
            <td style="font-size:0.9rem">: <?= date('d M Y', strtotime($bkk['tgl'])); ?></td>
       
    </tr>
    <tr>
        
        <td style="font-size:0.9rem">Pemasok</td>
        <td style="font-size:0.9rem">: <?= $bkk['pemasok'] ?></td>
...
<table style="position:absolute;margin-left:400px;margin-top:-70px;">
                <tr>
                    ...
                    <td style="font-size:0.9rem;">: <?= $bkk['gl_account'] ?></td>
                </tr>
                <tr>
                    <td style="font-size:0.9rem;">Deskripsi</td>
                    <?php foreach($account as $acc){
                        if($acc['no_account'] == $bkk['gl_account']){ ?>
                ...
                <table width="100%">
                    <tr>
                        ...
                        foreach ($detailbbkk as $detail) : 
                            if ($detail['kode_bbkk'] == $bkk['no_dok']){    
                        ?>
...
<table>
    <tr>
        ...
        <td style="font-size:0.9rem">: Rp.<?= number_format($bkk['bayar'], 0, ',', '.') ?></td>
   
    </tr>
    <tr>
        <td style="font-size:0.9rem">Terbilang</td>
        <td style="font-size:0.9rem">: <?= strtoupper(terbilang($bkk['bayar'])) ?> RUPIAH</td>
    </tr>
</table>
<table align="right">
        <tr align="center">
